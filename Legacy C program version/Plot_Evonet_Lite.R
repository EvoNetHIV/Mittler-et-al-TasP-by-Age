...
parameters <- read.table("parameters.txt",header <- FALSE, row.names <- NULL)
...
md_u25 <- ns[,"md_u25"] ; md_Mid <- ns[,"md_Mid"] ;md50 <- ns[,"md50"]
...
if (parameters[which(parameters[,2]=="plt_R"),1] == 1) {
...
if (parameters[which(parameters[,2]=="plt_AH"),1] == 1) {
 ...
 if ( (Group1end - Group1start >0 ) || (Group2end - Group2start > 0)) {
  ...
  for (ii in 1:length(unique_agents)) {
     jj <- which(ah[,"Agent"] == unique_agents[ii]) # jj is a list of all rows within AgentHistory for agent ii
     plot(ah_time[jj],log10(ah_V[jj]),col="red",type="l",lwd=2, xlab = "Time (Years)", ylab="VL (red), CD4 (blue)",
          main = paste("Treatment history for agent ",unique_agents[ii],
                        " [ Infected at year ",round(ah_tInf[jj][1],1),
                        " when ",round(ah_age[jj][1]),
                       " yrs old, P_drop = ",
                       round(100*365*ah_drop[jj][1],1),"% ]",
                       sep=""),
          ylim=c(0,7))
     lines(ah_time[jj],ah_cd4[jj],type="l",lwd=2,col="blue")
     lines(ah_time[jj],log10(ah_spvl[jj]),type="l",lwd= 0.4,col="grey")
     lines(ah_time[jj],8*ah_tx[jj]-1,type="p",lwd="3",pch=15,col="purple")
   }
 }
}
