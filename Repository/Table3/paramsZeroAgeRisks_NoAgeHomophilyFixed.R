...
evoparams$trans_lambda  =  0.000247 * 15  # some value times Hughes et al. (b/c they may be biased)  X3 to get a more robust epidemic
...
evoparams$trans_RR_age         = 1.0  # DIFFERENT FOR THIS EXPERIMENT
...
evoparams$prob_sex_by_age  = FALSE
...
D1 = 3*365     # Duration preference for people in group 1-1 pairs (short-term relationships)
D2 = 3*365  # Duration preference for people in group 2-2 pairs (long-term relationships)
...
evoparams$nw_form_terms <- "~edges + nodemix('att1', base=1) + concurrent(by='sex')  +offset(nodematch('sex', diff=FALSE))"
...
#  G2                E*f2*f2
 evoparams$target_stats <- c(tot_edges, 2*tot_edges*f1*f2, tot_edges*f2*f2,                       concur_women, concur_men)  # Original idea
...
modules <- c(
  ...
  "cd4_update", # viral_update_cd4_daily
  "coital_acts",
  "transmission",
  "deaths",
  "births",
